...
โปรเจกต์นี้มีเป้าหมายเพื่อแก้ปัญหา **Chatbot ตอบไม่ตรงคำถาม (High Fallback Rate)** โดยออกแบบกระบวนการจัดการข้อมูล (**Data Pipeline**) ที่เน้นผลลัพธ์ทางธุรกิจ ตั้งแต่การเก็บรวบรวมข้อมูล (Collection), การเตรียมข้อมูลภาษาไทยด้วย **Python** (Preparation), ไปจนถึงการวิเคราะห์และตัดสินใจผ่าน **Power BI Dashboard** (Analysis & Decision)

---

## 🧭 ผลลัพธ์ทางธุรกิจ (Executive Summary)

| ตัวชี้วัด (Metric) | ก่อนทำ (Manual) | หลังทำ (Automated) | การเปลี่ยนแปลง (Improvement) |
| :--- | :--- | :--- | :--- |
| **Understanding Rate**<br>*(อัตราที่บอทตอบได้)* | 75.0% | **88.9%** | 🔺 ดีขึ้น +13.9% |
| **Fallback Rate**<br>*(อัตราที่บอทตอบไม่ได้)* | 25.0% | **11.1%** | 🔻 ลดลง -13.9% |
| **Workflow Efficiency**<br>*(ประสิทธิภาพการทำงาน)* | Manual Excel | **Python Automation** | ⚡ ลดงานซ้ำซ้อนอย่างมีนัยสำคัญ |

> **📝 นิยามตัวชี้วัด (Metric Definitions):**
> * **Understood (เข้าใจ):** นับเป็น 1 เมื่อคำตอบของบอท **ไม่เข้าเงื่อนไข** Fallback / คำไม่สุภาพ / หรือการเลือกซ้ำ (ตรวจสอบโดยใช้ Regex-based Labeling Logic)
> * **Understanding Rate:** คำนวณเป็นเปอร์เซ็นต์จากจำนวนข้อความทั้งหมด (Message-weighted calculation)

---

## 🛑 ปัญหาและทางแก้ (Problem & Solution)
* **The Pain Point (ปัญหา):** ทีมงานขาดข้อมูลเชิงลึกว่าผู้ใช้ถามอะไรบ้าง เนื่องจาก Log เดิมเป็นข้อความดิบ (Unstructured Text) ที่ยากต่อการตรวจสอบ
* **The Solution (ทางแก้):** พัฒนากระบวนการ **Auto-Labeling** ด้วย Python เพื่อจัดกลุ่มหัวข้อสนทนาอัตโนมัติ ทำให้เห็น Insight และจุดที่ต้องปรับปรุงได้ทันที

---

## 🏗️ กระบวนการทำงาน (Data Analytics Process)

**(1) Data Collection (การเก็บรวบรวมข้อมูล)**
* รวบรวมข้อมูล Log การสนทนาจาก LINE OA และ Dialogflow
* จัดเก็บข้อมูลดิบลงใน Google Sheets เพื่อใช้เป็นฐานข้อมูลกลาง (Centralized Data Source)

**(2) Data Preparation (การเตรียมและทำความสะอาดข้อมูล)**
* **Data Extraction:** ดึงข้อมูลเข้าสู่ Python เพื่อเตรียมประมวลผล
* **Data Transformation:**
    * **Auto-Labeling:** ใช้ **Python Regex** สร้างกฎ (Rule-based) เพื่อจัดกลุ่ม Intent ภาษาไทย เช่น "ไม่เข้าใจ", "ข้อเสนอแนะ"
    * **Performance Tuning:** ใช้เทคนิค **Vectorization (`numpy.select`)** แทนการวน Loop เพื่อให้ประมวลผลข้อมูลจำนวนมากได้อย่างรวดเร็ว
* **Data Loading:** ส่งออกข้อมูลที่ Clean แล้ว (Structured Data) เพื่อนำไปวิเคราะห์ต่อ

**(3) Analysis & Visualization (การวิเคราะห์และแสดงผล)**
* ออกแบบ **Star Schema** ใน Power BI เพื่อเชื่อมโยงข้อมูลพฤติกรรมการแชท (Behavior) เข้ากับข้อมูลผู้ใช้งาน (Demographics)
* สร้าง **Interactive Dashboard** เพื่อติดตาม KPIs สำคัญและเจาะลึกถึงสาเหตุของปัญหา

---

## 💻 ไฮไลท์ทางเทคนิค (Implementation Highlights)

### A. Automated Categorization Logic (Python)
> *ตัวอย่าง Logic การจัดกลุ่มข้อความอัตโนมัติ โดยใช้ NumPy และ Regex เพื่อประสิทธิภาพสูงสุด*
...
# กำหนด Pattern คำหลักภาษาไทย (Thai Regex Patterns)
...
def label(series):
    ...
    # ใช้ np.select เพื่อประมวลผลทั้งคอลัมน์ในครั้งเดียว (Vectorization)
    return np.select(conds, choices, default="เข้าใจข้อคำถาม")

# สร้าง Flag เพื่อคำนวณ KPI
